...
@section('title', 'เพิ่มชุดใหม่ - ระบบร้านเช่าชุด')
    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header">
                <h4><i class="fas fa-plus me-2"></i>เพิ่มชุดใหม่</h4>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('admin.costumes.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-select @error('category') is-invalid @enderror" id="category" name="category">
                                ...
                                <option value="ชุดไทย" {{ old('category') == 'ชุดไทย' ? 'selected' : '' }}>ชุดไทย</option>
                                <option value="ชุดสากล" {{ old('category') == 'ชุดสากล' ? 'selected' : '' }}>ชุดสากล</option>
                                <option value="ชุดงาน" {{ old('category') == 'ชุดงาน' ? 'selected' : '' }}>ชุดงาน</option>
                                <option value="ชุดแฟนซี" {{ old('category') == 'ชุดแฟนซี' ? 'selected' : '' }}>ชุดแฟนซี</option>
                            </select>
                            @error('category')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" class="form-control @error('size') is-invalid @enderror" 
                                   id="size" name="size" value="{{ old('size') }}" placeholder="เช่น S, M, L, XL">
                            @error('size')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" class="form-control @error('color') is-invalid @enderror" 
                                   id="color" name="color" value="{{ old('color') }}" placeholder="เช่น แดง, น้ำเงิน">
                            @error('color')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-text">รองรับไฟล์: JPEG, PNG, JPG, GIF ขนาดไม่เกิน 2MB</div>
                        @error('image')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                        ...
                        <button type="submit" class="btn btn-primary">
                            <i class="fas fa-save me-2"></i>บันทึกชุด
                        </button>
                    </div>
                </form>
            </div>
